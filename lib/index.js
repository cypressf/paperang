 ...
 * @param {number} index
 */
async function fetchArticleFromRSS(rssUrl, index) {
  // Fetch the RSS feed
  const response = await fetch(rssUrl);
  const xmlData = await response.text();
  const result = await parseStringPromise(xmlData);
  const firstItem = result.rss.channel[0].item[index];
  const url = firstItem.link[0];
...
const print = async (/** @type {string} */ text) => {
...
const article = await fetchArticleFromRSS('https://www.cambridgeday.com/feed/', 3);
await print(article);
